  function pratinjau(event) {
    if(event.target.files.length > 0) {
      var src                 = URL.createObjectURL(event.target.files[0]);
      var prev                = document.getElementById('imgPratinjau');
      prev.src                = src;
  ...
  $(function() {
                        ...
                        if(key === "action") {
                            ...
                            if($("#"+key).prop("tagName") === "SELECT") {
                                ...
                                for(let i = 0; i < val.length; ++i) {
                                    $("#"+key).append("<option value='"+Object.keys(val[i])+"'>"+Object.values(val[i])+"</option>");
                                ...
                                if(isURL(val)) {
                                    $("#"+key).attr("src", val).css("width", "100%").css("height", "160");
                                }
    ...
    $("#btnRetrieve").click(function(event) { // Edit Data    
    ...
    $("#btnActivate").click(function(event) {  // Mengaktifkan Data       
        const $id    = $.map($table.bootstrapTable("getSelections"), (row) => {
            return row.id;
        });
        let count = Object.keys($id).length;
    ...
    $("#btnDeactivate").click(function(event) {  // Nonaktikan Data       
        const $id    = $.map($table.bootstrapTable("getSelections"), (row) => {
            return row.id;
        });
        let count = Object.keys($id).length;
    ...
    $("#btnDestroy").click(function(event) { // Hapus Data
    ...
    $("#btnRedir").click(function(event) { // Edit Data Redirect Page
        const $id   = $.map($table.bootstrapTable("getSelections"), (row) => {
            return row.id;
        });
        let count = Object.keys($id).length;
    ...
    $("#btnNavEdit").click(function(event) { // Edit Navbar    
                    ...
                    for(const [key, val] of Object.entries(data)) {
                        if(key === "action") {
                            ...
                            if($("#"+key).prop("tagName") === "SELECT") {
                                ...
                                if(isURL(val)) {
                                    $("#"+key).attr("src", val).css("width", "100%").css("height", "200");
                                }
                            } else if($("#"+key).prop("tagName") === "TEXTAREA") {
                                for( var instance in CKEDITOR.instances ) {
                                    CKEDITOR.instances[instance].setData(val);
